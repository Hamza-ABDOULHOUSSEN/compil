...
public class TDS
{
    ...
    public int addEntry(TDSEntry entry) { entries.add(entry); return entries.size(); }

	public String toString()
	{
        //return entries.stream().map(TDSEntry::toString).collect(Collectors.joining("\n", "{ ", " }"));		
		return toString2(0);
    }
	...
	public String toString2(int ind) {
		...
		for (TDSEntry entry: entries) {
			if (entry instanceof TDSIfEntry) {
				ret += strind + "containing following TDS (IfThen/TfThenElse):\n";

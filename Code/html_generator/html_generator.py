...
def start_buffer_write(start_buffer, path):
    ...
    for line in lines:
        if (k==6):
            start_buffer = start_buffer + '<title>'+path+'</title>'
        start_buffer += line
        k+=1
    fichier.close()
    return start_buffer
...
def add_title(buffer, basename):
    buffer = buffer + '\t<h2><span style="color: #3366ff;">Test : ' + basename + '</span></h2>\n\n'
    return buffer
...
def add_ast(buffer, file):
    ...
    buffer = buffer + '\t\t<img style="width:auto; height:auto;" src="../../ast/svg/'+ file + '.svg" class="table" >\n'
    buffer = buffer +  '\t</div>\n'
    return buffer

def add_tds(buffer, file):
    buffer = buffer + '\t<p>&nbsp;</p>\n\t<h3 id="tds"><span style="color: #333333;">TDS</span></h3>\n\t<p>&nbsp;</p>\n\t<div class="img-container">\n'
    buffer = buffer + '\t\t<img style="width:auto; height:auto;" src="../../tds/svg/'+ file + '.svg" class="table" >\n'
    buffer = buffer +  '\t</div>\n'
    return buffer
